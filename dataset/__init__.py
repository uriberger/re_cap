...
from dataset.caption_dataset import re_train_dataset, re_eval_dataset, pretrain_dataset_4m, coco_dataset, general_caption_dataset, pascal_dataset, nocaps_dataset
from dataset.nlvr_dataset import nlvr_dataset
from dataset.ve_dataset import ve_dataset
from dataset.vqa_dataset import vqa_dataset, vqa_local_dataset
...
def create_dataset(dataset, config, epoch=None):
    ...
    if dataset=='pretrain':
        ...
        train_dataset = vqa_local_dataset(config['local_train_file'], train_transform, split='train', read_local_data=config['read_local_data'], add_ocr=config['add_ocr'], add_object=config['add_object']) 
        vqa_test_dataset = vqa_local_dataset(config['local_test_file'], test_transform, split='test', read_local_data=config['read_local_data'], add_ocr=config['add_ocr'], add_object=config['add_object'])       
        vqa_val_dataset = vqa_local_dataset(config['local_val_file'], test_transform, split='test', read_local_data=config['read_local_data'], add_ocr=config['add_ocr'], add_object=config['add_object'])       
        return train_dataset, vqa_val_dataset, vqa_test_dataset
    elif dataset== 'nocaps':
        ...
        train_dataset = coco_dataset(config['train_file'], train_transform, config['coco_root'], max_words=config['max_length'], is_train=True)
        val_dataset = pascal_dataset(config['pascal_file'], test_transform, config['pascal_root'], max_words=config['max_length'], is_train=False)
